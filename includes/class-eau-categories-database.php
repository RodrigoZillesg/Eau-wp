...
/**
 * Gerencia a tabela de categorias de atividades
 */
class Eau_Categories_Database {

    /**
     * Nome da tabela
     */
    const TABLE_NAME = 'eau_activity_categories';
    ...
    public static function create_table() {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS $table_name (
            ...
            points_per_hour decimal(10,2) NOT NULL DEFAULT 1.00,
    ...
    public static function get_categories($args = array()) {
        ...
        // Order by
        $allowed_orderby = array('id', 'category_serial', 'category_name', 'points_per_hour', 'created_at');
    ...
    public static function save_category($data) {
        ...
        $category_data = array(
            ...
            'points_per_hour' => floatval($data['points_per_hour']),
        );
    ...
    /**
     * Busca categorias distintas das atividades atuais
     *
     * @return array
     */
    public static function get_distinct_categories_from_activities() {
        global $wpdb;
        ...
        $sql = "SELECT DISTINCT
                ...
                AND pm1.meta_key = 'act_category_serial'
                AND pm2.meta_key = 'act_category'
                AND pm1.meta_value != ''
                AND pm2.meta_value != ''
    ...
    /**
     * Sincroniza categorias das atividades com a tabela
     * Adiciona novas categorias encontradas que ainda não existem
     *
     * @return array Estatísticas da sincronização
     */
    public static function sync_categories_from_activities() {
        global $wpdb;

        $table_name = $wpdb->prefix . self::TABLE_NAME;
        $activity_categories = self::get_distinct_categories_from_activities();

        $stats = array(
            'total_found' => count($activity_categories),
            'added' => 0,
            'skipped' => 0,
        );

        foreach ($activity_categories as $activity_cat) {
            // Verifica se já existe
            $exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $table_name WHERE category_serial = %s",
                $activity_cat['category_serial']
            ));
            ...
            if (!$exists) {
                ...
                $result = $wpdb->insert(
                    $table_name,
                    array(
                        'category_serial' => $activity_cat['category_serial'],
                        'category_name' => $activity_cat['category_name'],
                        'points_per_hour' => 1.00, // Default
                    ),
                    array('%s', '%s', '%f')
                );
